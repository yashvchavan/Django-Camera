...
@login_required
def compare_cameras(request, camera_ids):
    """
    View for comparing multiple cameras
    Expects camera_ids in format like "1-2-3" or "1,2,3"
    """
    try:
        # Handle both hyphen and comma separated IDs
        ids = [int(id) for id in camera_ids.replace(',', '-').split('-')]
        cameras = Camera.objects.filter(id__in=ids)
